...
# preview_host: unlike the voice-synth addons (DX7/JV-880/Maze/Acid), it
# only does anything while this page is open (pad preview), so it's
# engine_autostart=1 instead of needing a manual POWER tap -- see
# force_shadow.c's own comment on that conf key. daemon.mjs itself stays
# boot-launched/always-on (it's just the shadow-page relay + kit editor
# backend, no voice slot of its own); this only toggles preview_host.
engine_process_name=preview_host
engine_nsmodule_path=/media/662522/AddOns/ForceKitBuilder/NSMODULE.json
engine_dirname=ForceKitBuilder
engine_arguments_json=[{"NAME":"ctrl-sock flag","VALUE":"--ctrl-sock"},{"NAME":"Kit Builder daemon's control socket path","VALUE":"/tmp/kitbuilder_ctrl.sock"},{"NAME":"mix-slot flag","VALUE":"--mix-slot"},{"NAME":"ForceAudioJack voice slot (0-3; 3, free -- 0=Maze Voice, 1=JV-880, 2=DX7)","VALUE":"3"}]
engine_autostart=1
